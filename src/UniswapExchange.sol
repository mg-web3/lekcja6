...
contract UniswapExchange is IUniswapExchange, ERC20 {
    /***********************************|
  |        Variables && Events        |
  ...
  |__________________________________*/
    ...
    function ethToTokenOutput(
        ...
        uint256 eth_sold = getOutputPrice(
            tokens_bought,
            address(this).balance.sub(max_eth),
            token_reserve
        );
        // Throws if eth_sold > max_eth
        uint256 eth_refund = max_eth.sub(eth_sold);
        if (eth_refund > 0) {
            (bool sent, ) = buyer.call{value: eth_refund}("");
    ...
    function tokenToEthInput(
        ...
        (bool sent, ) = recipient.call{value: wei_bought}("");
        require(sent, "Failed to send Ether");
    ...
    function tokenToEthOutput(
        ...
        (bool sent, ) = recipient.call{value: eth_bought}("");
        require(sent, "Failed to send Ether");
        require(token.transferFrom(buyer, address(this), tokens_sold));
    ...
    function tokenToTokenInput(
        ...
        require(token.transferFrom(buyer, address(this), tokens_sold));
        uint256 tokens_bought = IUniswapExchange(exchange_addr)
    ...
    function tokenToTokenOutput(
        ...
        require(exchange_addr != address(this) && exchange_addr != address(0));
        uint256 eth_bought = IUniswapExchange(exchange_addr)
            .getEthToTokenOutputPrice(tokens_bought);
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 tokens_sold = getOutputPrice(
            eth_bought,
            token_reserve,
        ...
        require(token.transferFrom(buyer, address(this), tokens_sold));
        IUniswapExchange(exchange_addr).ethToTokenTransferOutput{
            value: eth_bought
  ...
  |__________________________________*/
    ...
    function addLiquidity(
        ...
        if (total_liquidity > 0) {
            ...
            uint256 token_reserve = token.balanceOf(address(this));
            uint256 token_amount = (msg.value.mul(token_reserve) / eth_reserve)
                .add(1);
